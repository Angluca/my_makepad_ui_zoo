...
live_design! {
    ...
    pub DemoTextInput = <UIZooTabLayout_B> {
        ...
        demos = {
            <H4> { text: "TextInput" }
            <UIZooRowH> {
                simple_textinput = <TextInput> {}
                simple_textinput_outputbox = <P> { text: "Output" }
            }

            <Hr> {}
            <H4> { text: "TextIuput, Disabled" }
            <TextInput> {
                empty_text: "Intline Label"
...
pub fn handle_actions(app: &mut App, cx: &mut Cx, actions:&Actions) {
    if let Some(txt) = app.ui.text_input(ids!(simple_textinput)).changed(&actions) {
        app.counter += 1;
        let lb = app.ui.label(ids!(simple_textinput_outputbox));
        lb.set_text(cx, &format!("{} {}", app.counter, txt));
    }
}
